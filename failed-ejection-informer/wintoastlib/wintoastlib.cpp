...
bool WinToast::initialize(_Out_ WinToastError* error) {
    ...
    if (FAILED(DllImporter::Wrap_GetActivationFactory(WinToastStringWrapper(RuntimeClass_Windows_UI_Notifications_ToastNotificationManager).Get(), &notificationManager))) {
        setError(error, WinToastError::DllImporterBroken);
        DEBUG_MSG(L"Error while initializing Dll Importer =(");
        return false;
    }
